class User < ApplicationRecord
  ...
  def self.top_users
    top_users_array = self.all.map { |user| {name: user.name, wins: user.play_sessions.select{ |play| play.user_win == true }.length } }

    top = top_users_array.sort_by { |user_hash| user_hash[:wins] }.reverse.take(10)
    top
  end
